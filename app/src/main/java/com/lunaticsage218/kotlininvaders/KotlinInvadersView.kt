...
class KotlinInvadersView(
    ...
    // The players ship
    private var playerShip: PlayerShip = PlayerShip(context, size.widthPixels, size.heightPixels)
    ...
    private var playerBullet = Bullet(size.heightPixels, 1200f, 40f)
    ...
    private fun prepareLevel() {
        ...
        for (i in 0 until maxInvaderBullets) {
            invadersBullets.add(Bullet(size.heightPixels))
        }
    }
    ...
    private fun update(fps: Long) {
        ...
        for (invader in invaders) {
            ...
            if (invader.isVisible) {
                ...
                if (invader.position.left > size.widthPixels - invader.width
        ...
        if (bumped) {
            ...
            for (invader in invaders) {
                ...
                if (invader.position.bottom >= size.heightPixels && invader.isVisible) {
                    lost = true
                }
            }
        }
        ...
        for (bullet in invadersBullets) {
            if (bullet.position.top > size.heightPixels) {
    ...
    override fun onTouchEvent(motionEvent: MotionEvent): Boolean {
        val motionArea = size.heightPixels - (size.heightPixels / 8)
        when (motionEvent.action and MotionEvent.ACTION_MASK) {
            ...
            MotionEvent.ACTION_MOVE-> {
                ...
                if (motionEvent.y > motionArea) {
                    if (motionEvent.x > size.widthPixels / 2) {
